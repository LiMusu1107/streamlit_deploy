...
#     def __init__(self, in_dim, out_dim, dropout_c , hid_dim = 100):
#         super().__init__()
#         self.clf = nn.Sequential(
#             nn.Linear(in_dim, hid_dim),
#             nn.LeakyReLU(0.25),
#             nn.Dropout(dropout_c),  # 新增的Dropout层
...
# def init_model_dict(num_view, num_class, dim_list, hgcn_dim,  dropout_e):
#     """
#     初始化模型字典
...
#     # 为每个视图创建编码器
#     for i in range(num_view):
#         model_dict["E{:}".format(i+1)] = GCN_E(dim_list[i], hgcn_dim, dropout_e)
    
#     # 创建融合分类器（输入维度是所有编码器输出的concatenation）
#     # 每个编码器输出50维，所以总输入维度是 50 * num_view
#     concat_dim = hgcn_dim[-1] * num_view
#     model_dict["C"] = Classifier_1(concat_dim, num_class)
    
#     return model_dict


def init_model_dict(num_view, num_class, dim_list, dim_he_list,  dropout_e, dropout_c = 0):
    """
    初始化模型字典
    ...
    # 为每个视图创建编码器
    for i in range(num_view):
        model_dict["E{:}".format(i+1)] = GCN_E(dim_list[i], dim_he_list, dropout_e)
    
    # 创建融合分类器（输入维度是所有编码器输出的concatenation）
    # 每个编码器输出50维，所以总输入维度是 50 * num_view
    concat_dim = dim_he_list[-1] * num_view
