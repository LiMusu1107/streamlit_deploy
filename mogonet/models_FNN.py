...
class NN_E(nn.Module):
    def __init__(self, in_dim, hid_dim, dropout):
        ...
        self.fc1 = nn.Linear(in_dim, hid_dim[0])
        self.fc2 = nn.Linear(hid_dim[0], hid_dim[1])
        self.fc3 = nn.Linear(hid_dim[1], hid_dim[2])
        self.dropout = dropout
...
class VCDN(nn.Module):
    def __init__(self, num_view, num_cls, hvcdn_dim, dropout_vcdn = 0):
        super().__init__()
        self.num_cls = num_cls
        self.model = nn.Sequential(
            nn.Linear(pow(num_cls, num_view), hvcdn_dim),
            nn.LeakyReLU(0.25),
            nn.Dropout(dropout_vcdn),  # 新增的Dropout层
...
def init_model_dict(num_view, num_class, dim_list, dim_he_list, dim_hc, dropout_rate=0.5, dropout_vcdn = 0):
    model_dict = {}
    for i in range(num_view):
        model_dict["E{:}".format(i+1)] = NN_E(dim_list[i], dim_he_list, dropout_rate)
        model_dict["C{:}".format(i+1)] = Classifier_1(dim_he_list[-1], num_class)
    if num_view >= 2:
        model_dict["C"] = VCDN(num_view, num_class, dim_hc, dropout_vcdn)
    return model_dict
