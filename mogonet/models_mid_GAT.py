...
# 这段代码实现了对线性层的权重使用 Xavier 正态分布初始化，并将偏置项初始化为0。这是一种常见的初始化方法，有助于神经网络在训练时更快收敛。
def xavier_init(m):
    if type(m) == nn.Linear:
...
class GraphAttentionLayer(nn.Module):
    ...
    def forward(self, inp, adj):
        ...
        # torch.mm 是 PyTorch 中用于执行 矩阵乘法 的函数
        N = h.size()[0]    # N 图的节点数
...
def init_model_dict(num_view, num_class, dim_list, hgcn_dim, dropout_e=0.05, dropout_c = 0, alpha = 0.25):
    """
    初始化模型字典
    ...
    """
    model_dict = {}
    # 实际上隐藏层只有一层，取隐藏层节点数列表的第一个
    # 为每个视图创建编码器
    for i in range(num_view):
        model_dict["E{:}".format(i+1)] = GraphAttentionLayer(dim_list[i], hgcn_dim[1], dropout_e, alpha)
...
# 这里只用了单头注意力层 GraphAttentionLayer, 而没用多头注意力层 GAT
# def init_model_dict(num_view, num_class, dim_list, dim_he_list, dim_hc, dropout_rate = 0.2):
#     model_dict = {}
#     for i in range(num_view):
#         model_dict["E{:}".format(i+1)] = GraphAttentionLayer(dim_list[i], dim_he_list[1], dropout_rate, 0.25) # 只取了隐藏层第一个参数
