...
class CNN2D_E(nn.Module):
    def __init__(self, in_dim, hgcn_dim, dropout, kernal_shape, maxpool_shape ,reshape_factor=50):
        ...
        # 2D卷积层
        # self.conv1 = nn.Conv2d(1, hgcn_dim[0], kernel_size=(5, 5), padding=2)  # 调整padding保持尺寸 5,5 2,2
        self.conv1 = nn.Conv2d(1, hgcn_dim[0], kernel_size=kernal_shape[0], padding=2) 
        # self.maxpool1 = nn.MaxPool2d(kernel_size=(2, 2), stride=2)
        self.maxpool1 = nn.MaxPool2d(kernel_size=kernal_shape[0], stride=2)

        # self.conv2 = nn.Conv2d(hgcn_dim[0], hgcn_dim[1], kernel_size=(3, 3), padding=1)
...
def init_model_dict(num_view, num_class, dim_list, dim_he_list, dim_hc,kernal_shape, maxpool_shape , gcn_dopout=0.3, reshape_factor=50):
    model_dict = {}
    for i in range(num_view):
        model_dict["E{:}".format(i+1)] = CNN2D_E(dim_list[i], dim_he_list, gcn_dopout, kernal_shape, maxpool_shape ,reshape_factor)
