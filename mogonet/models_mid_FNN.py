...
# class Classifier_1(nn.Module):
#     def __init__(self, in_dim, out_dim, dropout_c = 0.1, hid_dim = 100):
#         super().__init__()
...
def init_model_dict(num_view, num_class, dim_list, hgcn_dim, dropout_e=0.05, dropout_c = 0):
    """
    初始化模型字典
    ...
    for i in range(num_view):
        model_dict["E{:}".format(i+1)] = NN_E(dim_list[i], hgcn_dim, dropout_e)
...
#     model_dict = {}
#     for i in range(num_view):
#         model_dict["E{:}".format(i+1)] = NN_E(dim_list[i], dim_he_list, gcn_dopout)
