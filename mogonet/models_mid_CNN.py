...
def init_model_dict(num_view, num_class, dim_list, hgcn_dim,  kernal_dim, maxpool_dim,dropout_e, dropout_c = 0):
    """
    初始化模型字典
    ...
    # 为每个视图创建编码器
    for i in range(num_view):
        model_dict["E{:}".format(i+1)] = CNN1D_E(dim_list[i], hgcn_dim,  kernal_dim, maxpool_dim, dropout_e)
